    ...
    return (
        ...
        <Text>¡Le recomendamos cambiar su contraseña para una mayor seguridad!</Text>
            ...
            validationSchema={Yup.object().shape({
                ...
                confirm_new_password: Yup.string().oneOf([Yup.ref('new_password'), ''], 'Las nuevas contraseñas deben coincidir')
                .required("Necesita reconfirmar la nueva contraseña"),
            })}
            onSubmit={async (values, actions) => {
                const { data, error } = await until(() => changePassword(user.id, values));
                ...
                if (error) {
                actions.setSubmitting(false);
                console.log("Error al intentar cambiar la contraseña", error);
                notifications.show({
                    color: "red",
                    title: "Error",
